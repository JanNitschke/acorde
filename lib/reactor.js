...
class Reactor {
    constructor(addr, port) {
        ...
        setInterval(() => {
            this.lastSend = process.hrtime();
            this.port.postMessage(new threads_1.ThreadStatsMessage(this.load, 0, this.actors.size));
        }, 100);
    }
    updateStats() {
        if (this.running) {
            ...
            this.load = mod_1.LOAD_SUMMERY_WEIGHT * this.load + (1 - mod_1.LOAD_SUMMERY_WEIGHT) * currentTimingNormal;
            this.lastLoadTest = process.hrtime();
            setTimeout(() => {
                this.updateStats();
            }, 1);
    ...
    async spawnActor(path, props) {
        ...
        const addr = new addr_1.Addr(this.reactorAddr.reactor + "." + "a" + (++this.lastActorId));
        const spawned = new actor_1.Actor(addr, this, mod);
    ...
    async sendMessage(msg) {
        ...
        if (isLocal) {
            ...
            this.port.postMessage(new threads_1.ThreadActorMessage(msg, id));
    ...
    async onWorkerMessage(msg) {
        if (msg instanceof threads_1.ThreadActorMessage) {
            ...
            this.port.postMessage(new threads_1.ThreadActorResponse(result, msg.id));
            return;
        }
        ...
        else if (msg instanceof threads_1.ThreadSpawnMessage) {
            const addr = await this.spawnActor(msg.path, msg.props);
            this.port.postMessage(new threads_1.ThreadSpawnedMessage(addr, msg.id));
            return;
        }
        else if (msg instanceof threads_1.ThreadDestroyMessage) {
            if (!msg.addr.actor)
